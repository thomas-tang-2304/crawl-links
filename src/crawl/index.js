...
const { mapParentIndex, filterOriginStatics } = require("./cheerio/utils");
...
const run = async (c_url, uid_socket) => {
      ...
      Object.entries(links).map(async ([key, value]) => {
        if (!key.includes("#")) {
          if (targetLinks.hasOwnProperty(key)) {
            targetLinks[key] = [...targetLinks[key], ...value];
          } else {
  ...
  Object.keys(ALL_LINK_LOAI.href_links).forEach((li) => {
    ALL_LINK_LOAI.href_links[li] = uniqueArray(ALL_LINK_LOAI.href_links[li]);
  });

  Object.keys(ALL_LINK_LOAI.src_links).forEach((li) => {
    ALL_LINK_LOAI.src_links[li] = uniqueArray(ALL_LINK_LOAI.src_links[li]);
  });
...
const runCrawling = async (Url, uid_socket) => {
  ...
  const result = await measureTime(() => run(parseUrl, uid_socket));

  return {
    a_href: {
      domain: filterOriginStatics(Object.keys(result.data.allLinks.href_links)),
      total: Object.keys(result.data.allLinks.href_links).length,
    },
    "*_src": {
      domain: filterOriginStatics(Object.keys(result.data.allLinks.src_links)),
      total: Object.keys(result.data.allLinks.src_links).length,
    },
  };
};
