...
const checkCrawlabledLinks = (thisLink, originUrl) =>
  ...
  !/\.(png|jpg|webp|avif|jpeg|gif|tiff|svg|pdf)$/i.test(thisLink); //ignore links which are containing image extension
...
const mapParentLink = (indexArray, refArray) => {
  ...
  return indexArray.map((queueLink) => refArray[queueLink]).map((link) => link.url);
};
...
module.exports = { mapParentIndex,mapParentLink, checkCrawlabledLinks, filterOriginStatics };